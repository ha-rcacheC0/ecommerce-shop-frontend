...
const EditShowForm: React.FC<EditShowFormProps> = ({ showId }) => {
  const navigate = useNavigate();

  // Fetch the existing show data
  ...
  // Initialize form with existing show data
  useEffect(() => {
    if (show) {
      setTitle(show.title);
      setDescription(show.description || "");
      setPrice(parseFloat(show.price).toString());
      setImage(show.image);
      setVideoURL(show.videoURL || "");
      setInStock(show.inStock);
      setShowTypeId(show.showTypeId);

      // Initialize selected products
      const products = show.ShowProducts.map((product) => ({
  ...
  if (showLoading || typesLoading || productsLoading) {
    return <div className="text-center p-8">Loading...</div>;
  }

  return (
    <div className="container mx-auto p-6 bg-base-100 rounded-lg shadow-lg">
      <h1 className="text-2xl font-bold mb-6">Edit Show: {show?.title}</h1>
